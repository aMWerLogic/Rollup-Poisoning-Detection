...
5. In results dir run analize_results.py for further analysis.
...
9. familiarity_and_payouts.py is used to gather payouts and remove cases where attacker and victim know each other and intend to send tokens between each other.
10. Run remove_dup_payouts.py to remove duplicate payouts.
11. payout_legit.py can be used on payouts to filter only those that involved a contract from out legitimate token list.
12. Save csv files as xlsx and perform neccessery manuall operations and statistics.